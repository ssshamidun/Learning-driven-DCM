...
- **Environment & Config**
  - `pointfoot_flat.py` — point-foot flat task (training-time privileged terrain heights, optional Vstab, differentiable stability loss).
  - `pointfoot_flat_config.py` — environment & training hyper-parameters (observations, rewards, curriculum, devices).
...
- **Overrides:** `--task`, `--resume`, `--experiment_name`, `--run_name`, `--load_run`, `--checkpoint`, `--num_envs`, `--seed`, `--max_iterations`.
...
```bash
python legged_gym/scripts/play.py   --task=pointfoot_flat   --load_run <your_model_folder>   --checkpoint <iter>
# example:
...
- **Practical training signals**: Normalized contact forces, commanded velocities, and training-only privileged terrain heights (no perception dependency at deployment).  
- **Point-foot environment**: Minimal, reproducible setup for flat and discrete-height (stair-like) scenarios; includes optional Vstab constraints.
...
- `pointfoot_flat.py` — Point-foot flat task, differentiable stability loss, Vstab options, privileged heights (training only).  
- `pointfoot_flat_config.py` — Env/training configs and curricula.
...
Built on the Isaac Gym stack and legged locomotion training pipelines; extended with a learning-driven DCM teacher, **offset-network optimization inside the runner**, and a **teacher imitation loss** in PPO.
